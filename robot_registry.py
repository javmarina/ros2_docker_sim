...
STANDARD_SCENARIOS = [
    ...
    Scenario(
        id="bash",
        name="Terminal bash interactivo",
        description="Inicia el contenedor en consola interactiva con todo el entorno de ROS 2 cargado."
    ),
]
...
def get_robot_by_name(name_str: str) -> Optional[RobotProfile]:
    ...
    for r in ROBOT_REGISTRY.values():
        if r.name == name_str or r.id in name_str.lower() or ("tiago" in name_str.lower() and r.id == "tiago"):
            return r
